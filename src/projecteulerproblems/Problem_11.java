...
public class Problem_11 {
    public Problem_11() {
        ...
        String digits = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 " +
    ...
    public int getLinePrd(String[] numbers, int position) {
        if(position % 20 > 16) {
            return 0;
        }
        int prd = 1, number;
        for (int i = 0; i < 4; i++) {
            number = Integer.parseInt(numbers[position + i]);
            prd *= number;
        }
        return prd;
    }
    
    public int getColumnPrd(String[] numbers, int position) {
        if(position / 20 > 16) {
            return 0;
        }
        int prd = 1, number;
        for (int i = 0; i < 4; i++) {
            number = Integer.parseInt(numbers[position + i * 20]);
            prd *= number;
        }
        return prd;
    }
